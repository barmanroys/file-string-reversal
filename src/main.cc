...
public:
    ...
    static void log(LogLevel level, const std::string& message) {
        ...
        std::cout << std::ctime(&nowTime) << " [" << levelStr << "] " << message << std::endl;
    }
};
...
public:
    virtual std::vector<std::string> get_original_lines() = 0; // Get the original strings to reverse 
    virtual void persist_reversed_strings(std::vector<std::string>) = 0; // Dump the reversed strings
};
...
public:
    ...
    virtual std::vector<std::string> get_out_strings() const = 0; // Get the reversed strings
};

class FDWithContext : public AbstractFileHandler
{ // Imitates a python context manager, to avoid forgetting to close the file
...
public:
    ...
    std::vector<std::string> get_original_lines()
    {
    ...
    void persist_reversed_strings(std::vector<std::string> lines)

    {
        for (const std::string &line : lines)  out_file->write(line);
...
void manipulate()
{
// Parallel execution of the join 
std::for_each(std::execution::par, lines.begin(), lines.end(), reverse_string);  
}


std::vector<std::string> get_out_strings() const
{
    return lines;
...
{
    std::string const in_file_path = "../data/in_file.txt";
    std::string const out_file_path = "../data/out_file.txt";
    auto io_handler = IOHandler(new FDWithContext(in_file_path, std::ios::in), 
    new FDWithContext(out_file_path, std::ios::in | std::ios::out | std::ios::app));
    LOG_INFO("IO Handler initialised.");
    auto lines = io_handler.get_original_lines();
    LOG_INFO("Read " + std::to_string(lines.size()) + " lines from the input file.");
    auto string_reverser = StringReverser(lines);
    LOG_INFO("String Reverser initialised.");
    string_reverser.manipulate();
    LOG_INFO("Reversed the strings.");
    io_handler.persist_reversed_strings(string_reverser.get_out_strings());
